...
router.get("/file/:id", async (req, res) => {
  try {
    const bucket = req.app.locals.bucket;
    const fileId = new mongoose.Types.ObjectId(req.params.id);  //url gives string
    //but mongodb needs objectid so conversion is required
    ...
    // ✅ FIX: fallback for old files where contentType was not saved
    const contentType = file.contentType ||
      (file.filename?.endsWith(".pdf") ? "application/pdf" : "application/octet-stream");
