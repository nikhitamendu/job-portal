...
/* ─────────────────────────────────────────────
   FONTS — only fonts + keyframes, no layout
───────────────────────────────────────────── */
const Fonts = () => (
  <style>{`
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="mono text-[10px] font-medium text-amber-400 tracking-widest uppercase">
                  Edit Listing
                </span>
              </div>
            ...
            {/* Status pill */}
            <span className={`inline-flex items-center gap-1.5 mono text-[10px] font-medium px-3 py-1.5 rounded-full border flex-shrink-0 ${
              form.isActive !== false
                ? "text-emerald-400 bg-emerald-400/10 border-emerald-400/25"
                : "text-slate-400 bg-slate-400/10 border-slate-400/25"
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full ${form.isActive !== false ? "bg-emerald-400" : "bg-slate-400"}`} />
              {form.isActive !== false ? "Active" : "Closed"}
            </span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 md:px-8 py-8">
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col gap-4">
            ...
            {/* ── Deadline Card ── */}
            <FormCard label="Application Deadline" delay="d5">
